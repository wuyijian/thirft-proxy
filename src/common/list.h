...
#ifdef  __cplusplus
#define list_for_each_entry(pos, head, member, T)				\
...
#ifdef  __cplusplus
#define list_for_each_entry_safe(pos, n, head, member, T) \
    for (pos = list_entry((head)->next, T, member), \
            n = list_entry(pos->member.next, T, member); \
         &pos->member != (head);  \
         pos = n, n = list_entry(n->member.next, T, member))
#endif
